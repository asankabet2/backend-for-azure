...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        // RequiredDocuments is stored as a JSON array string; expose it as a real array
        try {
...
router.put('/:id', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        // Fetch previous status so we only notify on transition TO Open
        ...
        // Only notify on transition TO Open (not on every save)
...
// ── DELETE /api/tenders/:id  — admin can only delete if status is Draft
...
router.post('/:id/interest', requireAuth, async (req, res) => {
    ...
    // supplier can only register interest for their own account
    ...
    try {
        ...
        // Notify admins that a supplier has expressed interest
...
router.get('/:tenderId/items/:itemNo/awards', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            existingAward: awardResult.recordset[0] || null, // back-compat (single award)
            existingAwards: awardResult.recordset,           // all awards (supports splits)
        });
    } catch (error) {
...
router.post('/:tenderId/items/:itemNo/award', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        // Items can only be awarded once the tender is Closed.
        const statusResult = await pool.request()
            .input('tenderId', sql.VarChar(50), tenderId)
            .query(`
                SELECT ts.TenderStatusName
                FROM Tender t JOIN TenderStatus ts ON t.TenderStatusID = ts.TenderStatusID
                WHERE t.TenderID = @tenderId
            `);
        if (statusResult.recordset[0]?.TenderStatusName !== 'Closed')
            return res.status(400).json({
                message: `Items can only be awarded once the tender is Closed. Current status: '${statusResult.recordset[0]?.TenderStatusName}'.`,
            });

        // Reject the award if the supplier's bid has been rejected.
        // A rejected bid disqualifies the supplier from being awarded any item.
        ...
        // A single award replaces any existing award(s) for this item — including a
        // previous split — so the item ends up with exactly one award row.
        await pool.request()
            .input('tenderId',     sql.VarChar(50), tenderId)
        ...
        // Count DISTINCT items (an item may now hold multiple split-award rows)
        await pool.request()
            .input('tenderId', sql.VarChar(50), tenderId)
            .query(`
                UPDATE Tender SET
        ...
        // Notify the awarded supplier about this item award
...
// Splits a single item's quantity across multiple suppliers. Rules:
//   • allocations must sum to EXACTLY the tender item quantity (full allocation)
//   • a supplier can't be awarded more than they bid for that item
//   • a rejected bid can't be allocated anything
router.post('/:tenderId/items/:itemNo/split-award', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        // Load every bid's quote for this item, keyed by BidID
        const bidsResult = await pool.request()
            .input('tenderId', sql.VarChar(50), tenderId)
            .input('itemNo',   sql.Int,         parseInt(itemNo))
            .query(`
        ...
        // Full allocation required (tolerate tiny decimal drift)
        ...
        // Mark the tender Awarded once every item has at least one award
        await pool.request()
            .input('tenderId', sql.VarChar(50), tenderId)
            .query(`
                UPDATE Tender SET
